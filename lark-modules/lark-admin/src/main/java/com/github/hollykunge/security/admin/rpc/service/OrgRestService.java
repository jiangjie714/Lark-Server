...
public class OrgRestService {
    ...
    public List<OrgTree> getTree(String userOrgCode, String grouptype)throws Exception{
        if(StringUtils.isEmpty(userOrgCode) || StringUtils.isEmpty(grouptype)){
            throw new BaseException("当前用户的组织id或类型不能为空");
        }
        Org org = orgMapper.selectByPrimaryKey(userOrgCode);
        if(org == null){
            throw new BaseException("当前用户组织为空...");
        }
        String pathCode = org.getPathCode();
        if(StringUtils.isEmpty(pathCode)){
            throw new BaseException("当前用户的组织无pathcode或组织无效..");
        }
        ...
        if(StringUtils.isEmpty(parentPathCode)){
            throw new BaseException("没有成功截取到上级pathcode...");
        }
...
//            userOrgCode = userOrgCode+Link;
//            return path.substring(0,path.length()-userOrgCode.length());
            return path.substring(0,getIndex(path,Link, Integer.parseInt(OrgLevelEnum.PARENT_DEPARTMENT.getValue())));
        }
        //二级
        if(Objects.equals(flag,OrgLevelEnum.SECOND_DEPARTMENT.getValue())){
            return path.substring(0,getIndex(path,Link, Integer.parseInt(OrgLevelEnum.SECOND_DEPARTMENT.getValue())));
        }
        //三级
        if(Objects.equals(flag,OrgLevelEnum.THIRD_PLACE.getValue())){
            return path.substring(0,getIndex(path,Link, Integer.parseInt(OrgLevelEnum.THIRD_PLACE.getValue())));
        }
        return null;
    }
    private int getIndex(String path,String str,int count){
        int i = 0;
        for(int y = 1;y <= count; y++){
            i = path.indexOf(str,i);
            i++;
        }
        return i;
    }
    private List<OrgTree> buildTree(List<Org> orgs, String parentTreeId, String flag) {
        ...
        for (Org org : orgs) {
            node = new OrgTree();
            node = JSON.parseObject(JSON.toJSONString(org), OrgTree.class);
