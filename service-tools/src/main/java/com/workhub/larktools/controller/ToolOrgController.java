...
public class ToolOrgController {
     ...
     * @MethodName: add
     * @Description: 增加组织节点
     * @Param: [params] 组织名称 ：name ，父节点pid,如果为空认为是根节点,order 排序,treeType树类型
    ...
    public ObjectRestResponse add(@RequestParam Map params) throws Exception{
       ...
       String treeId = toolOrgService.add(params);
       if(treeId.equals("0")){
           return res.rel(false).data("该类型的组织已经存在");
       }
       if(treeId.equals("-1")){
           return res.rel(false).data("用户所属场所信息无法查询");
       }

        return res.data(treeId);
    }
