...
public class OrgAndUserMqSendScheduled {
    ...
    public void refreshOrgMq(){
        ...
        if (eryuanOrgs.size() != 0) {
            ...
            eryuanOrgs.stream().forEach(eryuanOrg -> {
                eryuanOrgDao.updateOrgIsSuccess("1",eryuanOrg.getCASIC_ORG_CODE());
            });
            log.info("将未成功同步org数据重新发送至admin队列。");
        }
        log.info("没有未同步成功的org数据(admin)。");
    }
    @Scheduled(cron = "0 0 1,6,13,18,22 * * ?")
    public void refreshUserMq(){
        Example example = new Example(EryuanUser.class);
        example.createCriteria().andEqualTo("isSuccess","0");
        ...
        if (eryuanUsers.size() != 0) {
            ...
            eryuanUsers.stream().forEach(eryuanUser -> {
                eryuanUser.setIsSuccess("1");
                eryuanUserDao.updateByPrimaryKeySelective(eryuanUser);
            });
            log.info("将未成功同步user数据重新发送至admin队列。");
        }
        log.info("没有未同步成功user数据(admin)。");
    }

    @Scheduled(cron = "0 0 0,6,13,18,21 * * ?")
    public void refreshOnedocOrgMq(){
        Example example = new Example(EryuanOrg.class);
        example.createCriteria().andEqualTo("onedocIsSuccess","0");
        List<EryuanOrg> eryuanOrgs = eryuanOrgDao.selectByExample(example);
        //创建org队列list
        List<AdminOrgVO> orgVOList = new ArrayList<AdminOrgVO>();
        //创建队列id标识
        ...
        if (eryuanOrgs.size() != 0) {
            ...
            //将orgList发送到队列
            produceSenderConfig.sendOnedocOrgList(ids.toString(),orgVOList);
            eryuanOrgs.stream().forEach(eryuanOrg -> {
                eryuanOrgDao.updateOrgOnedocIsSuccess("1",eryuanOrg.getCASIC_ORG_CODE());
            });
            log.info("将未成功同步org数据重新发送至onedoc队列。");
        }
        log.info("没有未同步成功的org数据(onedoc)。");
    }
    @Scheduled(cron = "0 0 1,6,13,18,22 * * ?")
    public void refreshOnedocUserMq(){
        Example example = new Example(EryuanUser.class);
        example.createCriteria().andEqualTo("onedocIsSuccess","0");
        List<EryuanUser> eryuanUsers = eryuanUserDao.selectByExample(example);
        //创建user队列list
        ...
        if (eryuanUsers.size() != 0) {
            ...
            //将userList发送到队列
            produceSenderConfig.sendOnedocUserList(ids.toString(),userVOList);
            eryuanUsers.stream().forEach(eryuanUser -> {
                eryuanUser.setOnedocIsSuccess("1");
                eryuanUserDao.updateByPrimaryKeySelective(eryuanUser);
            });
            log.info("将未成功同步user数据重新发送至onedoc队列。");
        }
        log.info("没有未同步成功user数据(onedoc)。");
    }
}
